...
- `#pointy.certificates.<id>` — one certificate per step that has subject parameters: `{ model, plan, request, certificate, target }`, built from the same applications document the check runs over. The `certificate` derivation discharges that step's incoming residuals against its producers' `outPath`s and exports the facts it observed (including the selected `|` branch per join-typed parameter); forcing those exports is the only import-from-derivation in the flow.
