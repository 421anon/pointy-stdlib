...
- `#pointy.certificates.<id>` — one certificate per step whose `outPath` evaluates: `{ model, plan, request, certificate, target }`, built from the same applications document the check runs over, with `output = steps.<id>` and the certificates of the step's own producers as `parents`. The `certificate` derivation checks the step's own output against its interface's declared output type and discharges its incoming residuals against its producers' certificates, exporting the facts it observed (including the selected `|` branch per join-typed parameter); a step's certificate succeeding means its output and its whole upstream closure are certified. Forcing those exports is the only import-from-derivation in the flow.
- `#checks.<system>.pointy-schema` — drift: the document the language builds from `semantic.source` equals the committed `semantic.schema` (`pointy-schema` rewrites the file).
...
- `#apps.<system>.pointy-schema` — `nix run .#pointy-schema` regenerates the committed document in place (it resolves the repo root itself).
